...
void CEvc::spawnNode( const int key )
{
	...
	// find earth position
	{
		...
		pickOrig = mPicking->unProject(width / 2, height / 2, 0);
	...
	case SPAWN_DEBUG_OBJECT6: 
		{
			...
			{
				creature->GenerateProgressive(fileNames[ idx], pos, 
...
/**
 ...
 @date 2014-02-10
*/
void CEvc::onSubstep(PxF32 dtime)
{
	if (m_IsApplyCustomGravity)
	{
		PxSceneWriteLock scopedLock(*mScene);
...
RenderMaterial* CEvc::GetMaterial(const PxVec3 &rgb, bool applyVertexColor) // applyVertexColor=true
{
	...
	RenderMaterial *newMaterial = SAMPLE_NEW(RenderMaterial)(*getRenderer(), rgb, opacity, doubleSided, id, NULL, true, false, false, applyVertexColor);
