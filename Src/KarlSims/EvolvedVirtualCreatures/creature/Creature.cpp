...
void CCreature::GenerateProgressive( CPhenotypeNode *currentNode, const genotype_parser::SExpr *expr )
{
	...
	for each (auto &joint in currentNode->m_Joints)
	{
...
/**
 ...
 @date 2013-12-06
*/
CPhenotypeNode* CCreature::GenerateByGenotype( CPhenotypeNode* parentNode, const genotype_parser::SExpr *pexpr, 
	const int recursiveCnt, const PxVec3 &initialPos, const PxVec3 *linVel, const bool isGenerateBody, const PxVec3 &randPos, 
	const float dimensionRate, const PxVec3 &parentDim, const bool IsTerminal ) 
	// isGenerateBody=true, randPos=Px(0,0,0), dimensionRate=1, randPos=PxVec3(0,0,0), IsTerminal=false
{
	...
	while (pConnectList)
	{
		...
		if (boost::iequals(connection->conType, "joint"))
		{
			...
			CPhenotypeNode *pChildNode = GenerateByGenotype( pNode, connection->expr, recursiveCnt-1, 
				nodePos, linVel, true, randPos, dimensionRate*0.7f, dimension, IsTerminal);
...
/**
 ...
 @date 2014-01-17
*/
CPhenotypeNode* CCreature::CreateBody(const genotype_parser::SExpr *expr, const PxVec3 &initialPos, const PxVec3 *linVel, 
	const PxVec3 &randPos, const float dimensionRate, const PxVec3 &parentDim, OUT PxVec3 &outDimension, 
...
/**
 ...
 @date 2013-12-19
*/
void CCreature::CreateJoint( CPhenotypeNode *parentNode, CPhenotypeNode *childNode, 
	genotype_parser::SConnection *connect, const PxVec3 &conPos )
{
	RET(!childNode);
	RET(!parentNode);
	...
	PxTransform tm0, tm1;
	tm0 = PxTransform(body->getGlobalPose().p);
	tm1 = PxTransform(child->getGlobalPose().p);
...
void CCreature::Move(float dtime)
{
	...
	if (m_pRoot && m_pRoot->m_pShapeRenderer)
	{
		...
		for each (auto &node in m_nodes)
		{
			...
			worldBound.minimum.x = std::min(worldBound.minimum.x, bound.minimum.x);
			worldBound.minimum.y = std::min(worldBound.minimum.y, bound.minimum.y);
			worldBound.minimum.z = std::min(worldBound.minimum.z, bound.minimum.z);
			worldBound.maximum.x = std::max(worldBound.maximum.x, bound.maximum.x);
			worldBound.maximum.y = std::max(worldBound.maximum.y, bound.maximum.y);
			worldBound.maximum.z = std::max(worldBound.maximum.z, bound.maximum.z);
		}
...
void CCreature::GenerateRenderComposition( CPhenotypeNode *node )
{
	...
	if (!node->m_pOriginalShapeRenderer)
	{
		...
		if (1 == rigidActor0->getShapes(&shape0,1))
		{
			...
			if (renderActor0)
			{
				RenderComposition *newRenderer = SAMPLE_NEW(RenderComposition)(*m_sample.getRenderer(), node->m_PaletteIndex, 
	...
	for each (auto joint in node->m_Joints)
	{
		...
		RenderComposition *newRenderer = SAMPLE_NEW(RenderComposition)(*m_sample.getRenderer(),
