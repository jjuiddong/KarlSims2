...
CGenotypeNode* evc::CreateGenotypeNode(CEvc &sample, const genotype_parser::SExpr *expr)
{
	...
	if (IsSensorNode)
	{ // Sensor
		node->m_renderNode = SAMPLE_NEW(RenderBoxActor)(*sample.getRenderer(), dimension);
	}
...
RenderBezierActor* evc::CreateTransition(CEvc &sample, CGenotypeNode *from, CGenotypeNode *to, const PxVec3 &color, const u_int order)//order=0
{
	...
	RenderBezierActor *arrow = SAMPLE_NEW(RenderBezierActor)(*sample.getRenderer(), points, color);
...
void RemoveGenotypeTreeRec(CEvc &sample, CGenotypeNode *node, set<CGenotypeNode*> &diagrams)
{
	...
	for each (auto conNode in node->m_connectDiagrams)
	{
...
PxVec3 evc::GetDiagramPosition(CGenotypeNode *parent, CGenotypeNode *dispNode, OUT u_int &order)
{
	...
	const float radius = std::max(dimension.x*2.f +1, 2.f);

	set<string> symbols;
	PxVec3 val(radius,0,0);
	for each (auto child in parent->m_connectDiagrams)
	{
		...
		const float height = std::max(childDimension.y*2.f +1, 2.f);
...
/**
 ...
 @date 2014-03-03
*/
void evc::GetJointTransform(const PxVec3 *pos, const genotype_parser::SConnection *joint, 
	OUT PxTransform &tm0, OUT PxTransform &tm1, const bool applyRandom)// applyRandom = false
{
...
PxVec3 evc::RandVec3( const PxVec3 &vec, const float rate )
{
	...
	PxVec3 dimRand(randfloat2()*dimRand1.x, randfloat2()*dimRand1.y, randfloat2()*dimRand1.z);
	if (randfloat2() > 0.5f) dimRand.x = -dimRand.x;
	if (randfloat2() > 0.5f) dimRand.y = -dimRand.y;
	if (randfloat2() > 0.5f) dimRand.z = -dimRand.z;
	return dimRand;
}
...
{
	PxVec3 val;
	val.x = std::max(vec0.x, vec1.x);
	val.y = std::max(vec0.y, vec1.y);
	val.z = std::max(vec0.z, vec1.z);
	return val;
}
	...
	set<CGenotypeNode*> &symbols, OUT PxTransform &out)
{
	...
	for each (auto &con in from->m_connectDiagrams)
	{
	...
	const bool isShowHighLight)
{
	...
	pickOrig = sample.GetPicking()->unProject(x, y, 0);
	sample.GetPicking()->computeCameraRay(orig, dir, x, y);
