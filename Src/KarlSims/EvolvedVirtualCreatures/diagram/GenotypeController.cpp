...
{
	for each (auto node in m_diagrams)
		node->Render();
	if (m_popupDiagrams)
...
PxVec3 CGenotypeController::LayoutRec(CGenotypeNode *node, set<CGenotypeNode*> &symbols, const PxVec3 &pos)
{
	...
	map<CGenotypeNode*,u_int> orders;
	for each (auto child in node->m_connectDiagrams)
...
void CGenotypeController::RemoveUnlinkDiagram()
{
	...
	for each (auto node in m_diagrams)
	{
		for each (auto con in node->m_connectDiagrams)
		{
	...
	for each (auto kv in nodesPointing)
	{
		if (kv.second > 0)
			continue;
