...
COrientationEditController::~COrientationEditController()
{
	...
	for each (auto cursor in m_allCursor)
		delete cursor;
	m_allCursor.clear();
}
	...
	case MODE_POSITION:
		{
			...
			pickOrig = m_sample.GetPicking()->unProject(x, y, 0);
	...
	case MODE_ROTATION:
		{
...

			{
				...
				pickOrig0 = m_sample.GetPicking()->unProject(x, y, 0);
				m_sample.GetPicking()->computeCameraRay(orig0, dir0, x, y);
				PxVec3 orig1, dir1, pickOrig1;
				pickOrig1 = m_sample.GetPicking()->unProject(m_ptOrig.x, m_ptOrig.y, 0);
				m_sample.GetPicking()->computeCameraRay(orig1, dir1, m_ptOrig.x, m_ptOrig.y);
