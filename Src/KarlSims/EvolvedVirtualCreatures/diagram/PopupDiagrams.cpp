...
CGenotypeNode* CPopupDiagrams::CreateBoxDiagram()
{
	...
	node->m_renderNode = SAMPLE_NEW(RenderBoxActor)(*m_sample.getRenderer(), PxVec3(shapeSize,shapeSize,shapeSize));
...
{
	set<string> symbols;
	for each (auto node in m_diagramController.GetDiagrams())
