...
void CScaleCursor::onSelectNode(CGenotypeNode *node)
{
	...
	else
	{
		...
		if (m_lines.empty())
		{
			m_lines.push_back(SAMPLE_NEW(RenderBezierActor)(*m_sample.getRenderer(), points1, m_colors[0]));
			m_lines.push_back(SAMPLE_NEW(RenderBezierActor)(*m_sample.getRenderer(), points2, m_colors[1]));
			m_lines.push_back(SAMPLE_NEW(RenderBezierActor)(*m_sample.getRenderer(), points3, m_colors[2]));
...
{
	PxVec3 orig, dir;
	m_sample.GetPicking()->computeCameraRay(orig, dir, x, y);
...
void CScaleCursor::EditScale(CGenotypeNode *node, const Int2 initialPos, const Int2 curPos, const AXIS axis)
{
	...
	PxVec3 orig0, dir0;
	PxVec3 pickOrig0 = m_sample.GetPicking()->unProject(initialPos.x, initialPos.y, 0);
	m_sample.GetPicking()->computeCameraRay(orig0, dir0, initialPos.x, initialPos.y);

	PxVec3 orig1, dir1;
	PxVec3 pickOrig1 = m_sample.GetPicking()->unProject(curPos.x, curPos.y, 0);
	m_sample.GetPicking()->computeCameraRay(orig1, dir1, curPos.x, curPos.y);
	...
	{
	case X_AXIS: 
		dimensionValue = std::max(dim.x + deltaLen, 0.01f);
		node->m_expr->dimension.x = dimensionValue;
		break;
	case Y_AXIS: 
		dimensionValue = std::max(dim.y + deltaLen, 0.01f);
		node->m_expr->dimension.y = dimensionValue; 
		break;
	case Z_AXIS: 
		dimensionValue = std::max(dim.z + deltaLen, 0.01f);
