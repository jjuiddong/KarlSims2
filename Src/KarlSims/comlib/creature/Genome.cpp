...
namespace evc 
{
	bool GetChromo_Sub(const genotype::sBody *pexpr, OUT vector<double> &chromo,
		INOUT map<string, int> &check);

	genotype::sBody* BuildChromo_Sub(const vector<double> &chromo, const int startIndex, OUT int &nextIndex,
		INOUT map<int, genotype::sBody*> &symTable);

	void Mutate_Sub(INOUT vector<double> &chromo, const int startIndex, OUT int &nextIndex, INOUT map<int, genotype::sBody*> &symTable);
}
...
// convert pexpr to chromo
 bool evc::GetChromo_Sub(const genotype::sBody *body, 
	 OUT vector<double> &chromo, INOUT map<string, int> &check)
{
	 return true;
...
// convert pexpr to chromo
bool evc::GetChromo(const cCreature *creature, const genotype::sBody *body, OUT vector<double> &chromo)
{
	map<string, int> check;
	GetChromo_Sub(body, chromo, check);

	const cNeuralNet *nn = creature->m_phRoot->m_neuralNet;
	RETV(!nn, true);
...
genotype::sBody* evc::BuildBody(const vector<double> &chromo, OUT vector<double> &weights)
{
	map<int, genotype::sBody*> symTable;
	int next;
	genotype::sBody *expr = BuildChromo_Sub(chromo, 0, next, symTable);
	std::copy(chromo.begin()+(next+1), chromo.end()-1, std::back_inserter(weights));
	return expr;
}


genotype::sBody* evc::BuildChromo_Sub(const vector<double> &chromo, const int startIndex, 
	OUT int &nextIndex, INOUT map<int, genotype::sBody*> &symTable)
{
	return NULL;
...
void evc::Mutate_Sub(INOUT vector<double> &chromo, const int startIndex, OUT int &nextIndex, 
	INOUT map<int, genotype::sBody*> &symTable)
{
	const int weightSize = (int)chromo.back();
...
{
	const int mumWeightSize = (int)mum.back();
	const int dadWeightSize = (int)dad.back();
