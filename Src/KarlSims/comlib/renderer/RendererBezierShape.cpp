...
void RendererBezierShape::SetBezierCurve(const vector<PxVec3> &points, const PxVec3 &color)//color=PxVec(0,0,0)
{
	...
	const PxU32 diffuseColor = m_renderer.convertColor(RendererColor((PxU8)diffuse.x, (PxU8)diffuse.y, (PxU8)diffuse.z));
	...
	if (m_vertexBuffer)
	{
		...
		{
			PxVec3 pos;
			bezier(pos, points, (float)i/(float)(NODE_COUNT-1));
			...
			if (i > 0)
			{
				...
				curve.normalize();
				quatRotationArc(q, PxVec3(1,0,0), curve);
			}
